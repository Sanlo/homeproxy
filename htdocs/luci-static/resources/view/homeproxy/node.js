...
function renderNodeSettings(section, data, features, main_node, routing_mode) {
	...
	o = s.option(form.MultiValue, 'outbounds', _('Outbounds'), _('choose outbounds.'));
	...
	o = s.option(form.Value, 'url', _('Test URL'), _('https://www.gstatic.com/generate_204 will be used if empty.'));
	o.depends('type', 'urltest');
	o.validate = function (section_id, value) {
		if (section_id && value) {
			try {
				var url = new URL(value);
				if (!url.hostname) return _('Expecting: %s').format(_('valid URL'));
			} catch (e) {
	...
	o = s.option(form.Value, 'interval', _('Interval'), _('The test interval. 3m will be used if empty.') );
	o.depends('type', 'urltest');
	o.modalonly = true;

	o = s.option(form.Value, 'tolerance', _('Tolerance'), _('The test tolerance in milliseconds. 50 will be used if empty.') );
	o.datatype = 'uinteger';
	o.depends('type', 'urltest');
	o.modalonly = true;

	o = s.option(form.Value, 'idle_timeout', _('Idle timeout'), _('The idle timeout. 30m will be used if empty.'));
	o.depends('type', 'urltest');
	o.modalonly = true;

	o = s.option(form.ListValue, 'default', _('Default outbound'), _('The first outbound will be used if default.'));
	o.load = function (section_id) {
		delete this.keylist;
		delete this.vallist;
	...
	_(
		'Interrupt existing connections when the selected outbound has changed.<br/>' +
